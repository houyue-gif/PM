...
export function TaskCenter({ mineOnly = false }: { mineOnly?: boolean }) {
  ...
  const myTaskFilter = useUIStore((s) => s.myTaskFilter);
  const setMyTaskFilter = useUIStore((s) => s.setMyTaskFilter);
  ...
  const tasks = useMemo(() => {
    ...
    if (!mineOnly) return source;
    const now = new Date();
    const today = now.toISOString().slice(0, 10);
    if (myTaskFilter === "owned") return source.filter((t) => t.assigneeId === "u1");
    if (myTaskFilter === "watching") return source.filter((t) => t.watcherIds.includes("u1"));
    if (myTaskFilter === "overdue") return source.filter((t) => t.assigneeId === "u1" && t.dueDate.slice(0, 10) < today && t.status !== "done");
    return source.filter((t) => t.assigneeId === "u1" && t.dueDate.slice(0, 10) === today);
  }, [data?.items, mineOnly, myTaskFilter]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        {mineOnly && (
          <select className="rounded-md border border-slate-200 px-2 py-1 text-sm" value={myTaskFilter} onChange={(e) => setMyTaskFilter(e.target.value as never)}>
            <option value="owned">我负责</option>
            <option value="watching">我关注</option>
        ...
        <div className="ml-auto flex gap-2">
          {(["list", "board", "calendar"] as const).map((v) => (
            <button key={v} onClick={() => setTaskView(v)} className={`rounded-md px-3 py-1 text-sm ${taskView === v ? "bg-slate-900 text-white" : "bg-slate-100"}`}>
              {v}
            </button>
          ))}
        </div>
      ...
      {!isLoading && !isError && tasks.length > 0 && taskView === "list" && (
      ...
      {!isLoading && !isError && tasks.length > 0 && taskView === "board" && (
                ...
                <div key={t.id} className="mb-2 rounded-lg border border-slate-200 p-2">
                  ...
                  <div className="mt-2 flex gap-2">
                    {columns.filter((x) => x !== c).slice(0, 1).map((next) => (
                      <button key={next} className="text-xs text-blue-600" onClick={async () => { await patchJson("/app/api/tasks", { id: t.id, status: next }); qc.invalidateQueries({ queryKey: ["tasks"] }); }}>
                        转为 {next}
                      </button>
                    ))}
                  </div>
      ...
      {!isLoading && !isError && tasks.length > 0 && taskView === "calendar" && (
...
function TaskDrawer({ task, onClose }: { task: Task; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-30 w-[34rem] overflow-y-auto border-l border-slate-200 bg-white p-4 shadow-2xl">
      ...
      <div className="mt-5"><p className="mb-2 text-xs text-slate-500">子任务</p><div className="mb-2 flex gap-2"><Input value={subtaskTitle} onChange={(e) => setSubtaskTitle(e.target.value)} placeholder="新增子任务" /><Button onClick={async () => { if (!subtaskTitle.trim()) return; await patchJson("/app/api/tasks", { id: task.id, subTaskTitle: subtaskTitle }); setSubtaskTitle(""); qc.invalidateQueries({ queryKey: ["tasks"] }); }}>添加</Button></div><div className="space-y-1">{task.subTaskIds.length ? task.subTaskIds.map((id) => <p key={id} className="rounded bg-slate-50 px-2 py-1 text-sm">子任务 #{id}</p>) : <p className="text-sm text-slate-500">暂无子任务</p>}</div></div>
      <div className="mt-5"><p className="mb-2 text-xs text-slate-500">附件区（P0 基础占位）</p><div className="rounded-lg border border-dashed border-slate-300 p-3 text-sm text-slate-500">拖拽上传将在 P1 完整实现</div></div>
      <div className="mt-5"><p className="mb-2 text-xs text-slate-500">评论区（支持 @提及）</p><div className="space-y-2">{comments?.items.map((c) => <div key={c.id} className="rounded-lg bg-slate-50 p-2 text-sm" dangerouslySetInnerHTML={{ __html: c.content.replace(/@(\w+)/g, '<span class="text-blue-600 font-medium">@$1</span>') }} />)}</div><div className="mt-3 flex gap-2"><Input value={comment} onChange={(e) => setComment(e.target.value)} placeholder="发表评论，支持 @Bruce" /><Button onClick={async () => { if (!comment.trim()) return; await postJson(`/app/api/tasks/${task.id}/comments`, { content: comment }); setComment(""); qc.invalidateQueries({ queryKey: ["comments", task.id] }); }}>发送</Button></div></div>
      <div className="mt-5"><p className="mb-2 text-xs text-slate-500">动态记录</p><div className="space-y-1">{activities?.items.map((a) => <p key={a.id} className="text-sm text-slate-600">• {a.message}</p>)}</div></div>
    </div>
  );
}
